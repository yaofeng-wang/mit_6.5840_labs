...
func GenericTest(t *testing.T, part string, nclients int, nservers int, unreliable bool, crash bool, partitions bool, maxraftstate int, randomkeys bool) {
	...
	//log.Printf("slept for 2 seconds to ensure leader elected\n")
	defer cfg.cleanup()
	...
	for i := 0; i < 3; i++ {
		...
		//log.Printf("slept for 5 seconds\n")
...
// Commented out TestSpeed3A because it should be expected to have 100ms/op as each of the 1000 append calls will block
// until each op is applied by the leader and that will take at least 1 heartbeat
